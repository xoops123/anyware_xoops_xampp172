...
my ${CC} = q[cl];
my ${LIBS} = q[];
my ${installbuilddir} = q[\xampplite\apache\build];
my ${APRUTIL_LIB_TARGET} = q[];
my ${bindir} = q[\xampplite\apache\bin];
my ${APRUTIL_SO_EXT} = q[dll];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${includedir} = q[\xampplite\apache\include];
my ${exec_prefix} = q[\xampplite\apache];
my ${datadir} = q[\xampplite\apache];
my ${APRUTIL_LIBNAME} = q[libaprutil-1.lib];
my ${libdir} = q[\xampplite\apache\lib];
